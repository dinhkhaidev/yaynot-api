...
const createCommentInDB = async (payload) => {
  return nestedComment.create({ payload }); // Fixed: commentModel → nestedComment
};
const findCommentInDB = async (commentId) => {
  return nestedComment.findById(commentId);
};
...
const findCommentLikeByUserCommentId = async ({ userId, commentId }) => {
  return commentLike.findOne({ userId, commentId }).lean();
};
